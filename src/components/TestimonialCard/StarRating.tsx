...
const StarRating = ({icon, rating}: StarRatingProps) => {
  const stars = useMemo(() => {
    return Array.from({ length: MAX_STARS }, (_, index) => (
      <div
        key={index}
        className={`border border-Silver dark:border-gray-15 bg-[#d4d4d4] dark:bg-gray-10 rounded-full p-1.5 xl:p-2 2xl:p-2.5 flex justify-center items-center ${index < rating ? 'opacity-100' : 'opacity-30'}`}
      >
