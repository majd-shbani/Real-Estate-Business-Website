...
const StepsCard = ({items} : steps) => {
    return (
        ...
        {items.map((item , index) => {
            return (
                <div key={index} className="h-full max-w-112.5 md:max-w-full ">
                    ...
                    <BaseCard className="relative h-[80%] rounded-tl-none flex flex-col gap-3.5 xl:gap-4 2xl:gap-5 bg-[linear-gradient(130.79deg,#703BF7_-49.01%,rgba(112,59,247,0)_14.65%)]">
                        ...
                        <p className="font-medium text-gray-60 text-sm lg:text-base 2xl:text-lg leading-normal">{item.subTitle}</p>
                    </BaseCard>
                </div> 
            )
        })}
