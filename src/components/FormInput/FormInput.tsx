...
const FormInput = ({input, isActive, showAction, onActionClick}: FormInputProps) => {
  ...
  return (
    <div className={`flex flex-col gap-3 xl:gap-3.5 2xl:gap-4 ${showAction ? 'cursor-pointer' : ''}`} onClick={showAction ? onActionClick : undefined}>
        ...
        <div className="flex items-center gap-1.5 2xl:gap-3 bg-gray-10 border border-gray-15 rounded-md 2xl:rounded-lg px-5 py-4 2xl:py-6">
          ...
          {input.type === "select" ? (
            <div className="relative w-full flex items-center">
              <select 
                ...
                className="text-sm 2xl:text-lg placeholder:text-gray-40 w-full focus:outline-none bg-transparent text-gray-40 appearance-none pr-8 z-10 cursor-pointer"
              >
            ...
            <textarea name={input.name} placeholder={input.placeholder} 
              className="h-14.5 xl:h-[6.111111vw] 2xl:h-[6.25vw] placeholder:font-medium placeholder:text-gray-40 placeholder:text-sm placeholder:2xl:text-lg placeholder:leading-5 w-full bg-transparent focus:outline-none resize-none" />
          ) : (
            <input type={input.type} name={input.name} placeholder={input.placeholder} disabled={isDisabled} required
              className="text-sm placeholder:font-medium placeholder:text-gray-40 placeholder:text-sm placeholder:2xl:text-lg placeholder:leading-5 w-full bg-transparent focus:outline-none" />
          )}
          {showAction && (
            <div
