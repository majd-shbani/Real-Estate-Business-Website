import { propertyKeyFeatures } from "../../data/PropertyKeyFeatures/PropertyKeyFeatures"


const PropertyKeyFeatures = () => {
  return (
    <div className=" flex flex-col lg:flex-row px-4 gap-5 lg:gap-5 2xl:gap-7.5 sm:px-[9%]  lg:px-[4%] xl:px-[8%]  py-[20px] ">

      <div className="border border-gray-15 rounded-xl p-5 xl:p-10 h-max  2xl:p-12.5 lg:w-[49%]">
        <div className="mb-5 xl:mb-10 2xl:mb-12.5   ">
        <h2 className=" font-semibold text-[18px] xl:text-[20px] mb-1.5 xl:mb-2.5 2xl:mb-3.5 2xl:text-[24px]">{propertyKeyFeatures.title}</h2>
        <p className="text-gray-60 text-[14px] xl:text-[16px] 2xl:text-[18px]">{propertyKeyFeatures.description}</p>
        </div>
        ...
        <div className="relative grid grid-cols-2 grid-rows-2 divide-y sm:divide-y-0 divide-gray-300 border-t sm:flex sm:divide-x sm:divide-gray-25 sm:justify-between border-gray-15  sm:pt-3 xl:pt-4 2xl:pt-7 ">
          ...
          {propertyKeyFeatures.propertyRooms.map((item) => {
            return (
